...
int main(int argc, char* argv[])
{
    ...
    std::thread resume_data_loader([&ses]
    {
        // load resume files
        lt::error_code ec;
        std::vector<std::string> ents = list_dir(path_append(save_path, ".resume")
            , [](lt::string_view p) { return p.size() > 7 && p.substr(p.size() - 7) == ".resume"; }, ec);
        if (ec)
        {
            std::fprintf(stderr, "failed to list resume directory \"%s\": (%s : %d) %s\n"
                , path_append(save_path, ".resume").c_str(), ec.category().name(), ec.value(), ec.message().c_str());
        }
        else
        {
            for (auto const& e : ents)
            {
                ...
                std::string const file = path_append(path_append(save_path, ".resume"), e);
